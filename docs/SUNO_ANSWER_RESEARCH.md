...
## Answers recorded (2026-09-04, batch 4 — final residuals, settled at build time)

| Ref | Answer |
|-----|--------|
| D3 | **Stream deadline 90s** (gateway `ANSWER_TIMEOUT` env, default 90s, per-request ctx deadline on the Gemini call). TTFB stays a soft target — no separate hard fail. Proxy/app deadlines stack outward: sidecar waits 100s (so the gateway's own kinder timeout event arrives), Swift waits 110s. `http.Server.WriteTimeout` raised 30s→150s — the write deadline covers the whole streaming response, and 30s killed healthy streams mid-flight. nginx `/answer` location: `proxy_buffering off`, `proxy_read/send_timeout 120s`, `client_max_body_size 4m`, HTTP/1.1 + `Connection ""`. |
| D8 | **Sidecar reads corrections itself; `relevant_for` keyed on the QUERY** (not a transcript), limit 40 → gateway re-caps at 64. Turn 1 payload = `{query, image(base64 JPEG), dictionary}`; turn n>1 = `{query, history:[{q,a}]}`. Image rides turn 1 only; history is text-only. |
| D5 impl | **Separate `answer_usage` SQLite table** (same `(key_id, day, count)` shape). Env: `ANSWER_QUOTA_RPM=5`, `ANSWER_QUOTA_DAILY=50`, `ANSWER_HARD_DAILY=100`. Separate `ratelimit.AnswerLimiter` chained auth→answer-limiter→handler; 429 body is exactly `{"error":"limit"}` + `Retry-After` (60 = daily, 1 = per-minute — the sidecar tells these apart: per-minute maps to an "unavailable, wait a few seconds" event, daily to the limit card). Counted at stream start (batch-2 consequence stands). |
| D2 impl | `/answer` `MaxBytesReader` 4MB; image part rejected over 3MB decoded (base64 of a 1600px JPEG lands ~550KB). Swift `ScreenShot.jpegData` also refuses >3MB (belt-and-braces, fails client-side). |
| D4 impl | SSE `error` event `{"error":code,"message":...}`; taxonomy `unavailable\|timeout\|limit\|unable` + `not_entitled` (402 JSON verbatim through the sidecar, opening the same account sheet as dictation). Per-minute-limit wording: "answering as fast as it can — wait a few seconds". No raw fallback anywhere. |
| D6 impl | `internal/research` package (prompt: framing rules → `[SCREENSHOT]` → `[CONVERSATION SO FAR]` → `[DICTIONARY]` → `[QUESTION]`, caps 2000/8 turns/4000/64) + `backend.AnswerBackend` interface (`StreamAnswer(ctx, prompt, imageJPEG) <-chan AnswerChunk`). Cleanup's `Backend` untouched. `RESEARCH_MODEL` env (default flash-lite) → `GeminiBackend.AnswerModel`; thinking level reuses `GEMINI_THINKING_LEVEL`. |
| D8/GW | Gateway keeps history ≤8 turns client-side of the prompt (oldest dropped), clips query to 2000 chars. DictionarySection exported from cleanup so both prompts render entries identically. |
| C6 impl | Grounding: `tools:[{"googleSearch":{}}]` always on. `groundingChunks[].web.domain` (URI-host fallback) deduped, www-stripped, capped 4 → `sources` SSE event `{domains, queries}`. A Gemini chunk can carry text AND grounding — both handled (text is not dropped). |
| D9 impl | Event `answer` with the batch-3 field list; `turn_index = len(history)`, `outcome ok\|timeout\|error\|aborted`, `aborted_at_ms` on abort, zero content. |
| SSE wire | Events: `meta` (lease, first, always) → `delta`×N → `sources` (0/1) → `done` | `error`. Sidecar proxies bytes byte-for-byte; pre-stream failures become an in-stream single `error` event (so the app parses one shape) EXCEPT 402-class refusals, which stay bare 402 JSON like /transcribe. |
| Query fix | **Sidecar corrects the dictated query against the dictionary BEFORE the gateway sees it** (2026-09-04): `corrections.apply()` — corrections only, never expansions (blind value-substitution can't tell "here's my LinkedIn" from "I don't have a LinkedIn"). In-process regex pass, microseconds — no extra round trip; gateway UNCHANGED (it receives an already-corrected query). When the query changed, the sidecar prepends ONE extra SSE event `query {"query":"<corrected>"}` before `meta`; unchanged → stream is still byte-for-byte gateway output. App-side: `AnswerClient` parses `.query`, `AnswerFlow` swaps `self.query` and `AnswerPopup.replaceLastUserTurn` revises the user bubble in place — the visible raw→corrected swap is the "we heard you right" feedback, and history stores the corrected question. |
| Abort | Swift cancels the URLSession task on dismiss; the sidecar's passthrough generator closes; gateway sees the disconnect (`r.Context().Err()`) → analytics `aborted` + no terminal event written. |

**Built & verified (2026-09-04):** gateway (research pkg, backend StreamAnswer, answer limiter, SSE handler, 90s deadline, 150s WriteTimeout) — go test green; sidecar proxy in BOTH copies (`sidecar/server.py` + `sidecars/shared/answer.py` + route in both apps) — 210 pytest green; Swift `AnswerClient.swift` (multipart + SSE parser + error mapping) and `AnswerFlow` rewire (history, streamTask abort, notEntitled → account sheet) — swift build green, StubStream deleted. Local E2E (gateway↔stub-Gemini↔curl): meta/delta/sources/done + `{"error":"limit"}` at 5rpm verified; sidecar→gateway E2E verified. **Deployed to VPS** (binary + nginx `/answer` SSE block, nginx -t clean, reload green); live checks: `/health` `/ready` ok, unauth `/answer` → 401, `/cleanup` regression unchanged. Real-key E2E through the paired account happens from the app (Keychain ACL blocks CLI reads — recorded gotcha).

Still open after this round: B6 (PII consent surface wording), E3–E7 (paid-tier keys, metadata-logging audit, retention), F1 (re-verify per-search pricing), F2 (confirm image-token budget with real screenshots), F4 (quota math vs. paid price). ~~D3, D8, D11~~ — settled in batch 4 (2026-09-04).
